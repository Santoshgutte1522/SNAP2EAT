...
// 1. Initialize the WhatsApp Client
const client = new Client({
  // FIX: This ensures the session persists on your friend's laptop
  authStrategy: new LocalAuth({
    dataPath: "./session",
  }),
  puppeteer: {
    // If on Render/Docker (Linux), we set null to let Puppeteer find the installed Chrome
    executablePath: isProduction
      ? null
      : "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe",
    headless: true, // Docker MUST be headless: true
    args: [
      "--no-sandbox",
      "--disable-setuid-sandbox",
      "--disable-dev-shm-usage",
